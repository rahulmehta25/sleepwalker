...
| 4. Deploy | 4/9 | In Progress (Waves 0 + 1 complete) | - |
| 5. Queue | 0/TBD | Not started | - |
| 6. Polish | 0/TBD | Not started | - |

---
*Roadmap created: 2026-04-18*
*Last updated: 2026-04-20 after Phase 4 Plan 04 execution (Wave 1 sealed — routines/actions.ts 4 deploy-family Server Actions landed; commits `c5a9c75` feat amend DeployState warning field + `d06d22b` feat routines/actions.ts 860 lines covering deployRoutine + getDeployState + runNowRoutine + setRoutineEnabled + 3 result-type unions + `8047e2e` test deploy-routine-action 9 blocks + `85cd378` test run-now-action 6 + set-enabled-action 6 blocks; dashboard suite 297 → 318 green across 34 files; typecheck exit 0; 13 VALIDATION rows flipped to `4-04 green` — rows 1+5+6+7+8 (state machine + rollback + nested-error + no-orphan + bootout-timeout) + 13+14+15+16 (per-runtime Run-now dispatch) + 17+18+19+20 (launchctl bootstrap/bootout + persist flag + enable draft error); 29/36 VALIDATION rows total green. Wave 1 COMPLETE; Wave 2 (04-05 save-to-repo Server Action wrappers + 04-06 HealthBadgeRow + 04-07 DeployProgressDrawer) unblocked — 04-05 serializes against 04-04 on actions.ts file writes, 04-06 + 04-07 parallel-safe with each other. DEPL-01/02/04/05 Server Action surfaces all code-complete; UI consumer plans 04-07/08/09 complete the end-to-end DEPL surface)*
