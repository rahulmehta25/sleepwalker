...
| 4. Deploy | 5/9 | In Progress (Waves 0 + 1 complete; Wave 2 partial) | - |
| 5. Queue | 0/TBD | Not started | - |
| 6. Polish | 0/TBD | Not started | - |

---
*Roadmap created: 2026-04-18*
*Last updated: 2026-04-20 after Phase 4 Plan 05 execution (Wave 2 first ship — save-to-repo Server Action wrappers landed on routines/actions.ts; commits `1ae5398` feat append previewSaveToRepoAction + commitSaveToRepoAction + releaseSaveLockAction + PreviewActionResult type alias as pure pass-throughs to @/lib/save-to-repo — never-push / never-sweep invariants propagate unchanged from lib → action layer because the wrapper holds no simpleGit instance; `659ef16` test save-to-repo-action 4 it() blocks real-git integration matrix against mkdtempSync repo with SLEEPWALKER_REPO_ROOT env override — preview returns ok+hex lockToken, preview→commit round-trip verified via git log --oneline -1, preview→release→preview re-allows second preview (flock freed), preview→preview without release returns kind:lock-busy; dashboard suite 318 → 322 green across 35 files; typecheck exit 0; REPO-01 Server Action surface now code-complete — Plan 04-08 SaveToRepoModal is fully dep-cleared. Wave 2 partial (04-05 done; 04-06 HealthBadgeRow client + 04-07 DeployProgressDrawer remain — parallel-safe with each other). Wave 3 (04-08 SaveToRepoModal) dep-cleared. Wave 4 (04-09 exit gate) gates on everything. Phase 4 now 5/9 plans done (56%). Pre-existing parallel-session uncommitted paths preserved untouched via explicit staging. Zero Rule 1/2/3 auto-fixes — wrapper is trivial pass-through; lib-level coverage in 04-02 is already authoritative)*
