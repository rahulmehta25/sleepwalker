...
| DEPL-05 | Phase 4 | Partial (04-04 setRoutineEnabled Server Action with launchctl bootstrap/bootout for codex/gemini + per-runtime persistEnabledFlag — codex/gemini rewrite config.json, claude-desktop delegates to v0.1 setEnabled, claude-routines flips archived_fleets with inverse semantics — and first-enable invariant refusing non-succeeded state; 04-08 ships the shared ConfirmDialog component (role=dialog aria-modal with focus-on-cancel + Esc/backdrop close + btn-danger destructive CTA) ready for consumption by the disable-routine toggle per UI-SPEC §Confirmation copy line 259; UI action-bar consumer in 04-09 wires the disable-toggle + ConfirmDialog and completes the surface; commits `d06d22b` + `b09ab93`) |
...
| REPO-01 | Phase 4 | Partial (04-02 ships save-to-repo.ts simple-git + proper-lockfile library surface with never-push / never-sweep invariants grep-verifiable at module boundary; 04-05 ships 3 Server Action wrappers previewSaveToRepoAction / commitSaveToRepoAction / releaseSaveLockAction as pure pass-throughs; 04-08 ships SaveToRepoModal two-stage Review→Confirm client component (555 lines) with owned flock lifecycle + DiffStatPanel diff render + ConfirmDialog Discard prompt + INPUT_OPT_OUT textarea + Cmd/Ctrl+Enter submit + never-push subtitle + post-commit toast `Committed {shortSha} — {message.split('\n')[0]}`; commits `55740f8` + `7279030` + `1ae5398` + `659ef16` + `b09ab93` + `feadcd6`; Plan 04-09 wires SaveToRepoModal trigger into routines-client.tsx for formal Complete at phase exit gate) |
...
*Last updated: 2026-04-20 after Phase 4 Plan 08 execution (Wave 3 sealed; REPO-01 flipped from Pending to Partial — 04-02 library + 04-05 Server Action wrappers + 04-08 SaveToRepoModal two-stage Review→Confirm client + DiffStatPanel diff render + ConfirmDialog Discard prompt all code-complete; DEPL-05 Partial note extended to mention the shared ConfirmDialog component is now ready for the disable-toggle consumer; 04-09 wires both surfaces into routines-client.tsx for formal Complete at phase exit gate. 15/32 requirements fully Complete unchanged; 6 Partials now tracked — DEPL-01 + DEPL-02 + DEPL-04 + DEPL-05 + REPO-01 + HLTH-01)*
