...
| 2-06-01 | 06 | 2 | ADPT-07 | V2/V12 | `codex.deploy` full flow: `assertValidSlug` → absolute path resolve via login shell → plist gen → `plutil -lint` → `launchctl bootstrap` | unit | `pnpm test -- codex.test.ts` (mock execFile + fs) | ✅ W2 | ✅ green (Plan 02-07, commit fbda124; deploy full-flow test block with plutil<bootout<bootstrap ordering + plist mode 0644 + OPENAI_API_KEY absence byte-level check) |
| 2-06-02 | 06 | 2 | ADPT-07 | V2 | `codex.healthCheck` detects auth conflict (subscription + env key); returns `{available: true, reason: "WARN: ..."}` (temporary encoding until Plan 09 adds `warning` field) | unit | same (fixture `~/.codex/auth.json` + `OPENAI_API_KEY`) | ✅ W2 | ✅ green (Plan 02-07, commit fbda124; healthCheck auth-conflict test block + no-warning-when-apikey-preferred + cli-missing + version-failed bonus blocks) |
| 2-06-03 | 06 | 2 | ADPT-07 | — | codex real-Mac launchctl bootstrap + kickstart + bootout | **manual** | `test/manual/codex-adapter-smoke.md` | ❌ W4 | ⬜ deferred to Plan 02-10 manual smoke |
| 2-07-01 | 07 | 2 | ADPT-08 | V2/V12 | `gemini.deploy` injects `GOOGLE_CLOUD_PROJECT` into plist `EnvironmentVariables` | unit | `pnpm test -- gemini.test.ts` | ✅ W2 | ✅ green (Plan 02-08, commit 72c6f69; deploy happy-path test reads plist XML bytes and asserts `<key>GOOGLE_CLOUD_PROJECT</key><string>my-test-project</string>` present + `GEMINI_API_KEY` absent + NO_COLOR present; also covers deploy-BLOCKED-on-missing-quota branch) |
| 2-07-02 | 07 | 2 | ADPT-08 | V2 | `gemini.healthCheck` probes quota project + `gemini --version`; surfaces `GOOGLE_APPLICATION_CREDENTIALS` vs `GEMINI_API_KEY` collision | unit | same | ✅ W2 | ✅ green (Plan 02-08, commit 72c6f69; healthCheck happy test asserts version contains `(quota: my-test-project)` + `[auth: google-signin]`; SAC+API-key conflict test asserts `reason.match(/^WARN: /)` + contains both env var names; missing-quota-project test asserts WARN surfaces even when CLI is healthy) |
| 2-08-01 | 08 | 3 | ADPT-09 | — | `ADAPTERS` registry exports real adapters (not stubs) keyed by runtime | unit | `pnpm test -- adapter-registry.test.ts` | ✅ W3 | ✅ green (Plan 02-09, commit 78eaaf7; adapter-registry.test.ts §"has exactly 4 keys" + §"each adapter's runtime discriminant matches" + §"no adapter still says 'not implemented'" — T-02-09-01 + T-02-09-03 mitigated) |
| 2-08-02 | 08 | 3 | ADPT-09 | — | `healthCheckAll()` returns exactly 4 `HealthStatus` objects in parallel | unit | same | ✅ W3 | ✅ green (Plan 02-09, commit 78eaaf7; adapter-registry.test.ts §"returns exactly 4 HealthStatus objects" under fail-all execFile mock — T-02-09-04 mitigated; Promise.all never rejects) |
