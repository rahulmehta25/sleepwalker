...
| TBD | TBD | 0 | DEPL-04 | — | gemini runNow same shape as codex | unit | `pnpm test tests/run-now-action.test.ts -t "gemini detached"` | ❌ Wave 0 | ⬜ pending |
| TBD | TBD | 0 | DEPL-05 | — | setRoutineEnabled(enabled=false) calls launchctl bootout | unit | `pnpm test tests/set-enabled-action.test.ts -t "disable bootout"` | ❌ Wave 0 | ⬜ pending |
