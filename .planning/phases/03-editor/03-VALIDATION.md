...
| TBD | TBD | 0 | EDIT-02 | SAFE-02 | `saveRoutine` secret-scans; Stripe / GitHub / AWS / 40-hex / OpenAI / Anthropic / Slack / Google / PEM all detected | unit | `cd dashboard && pnpm test secret-scan.test.ts` | ❌ Wave 0 | ⬜ pending |
...
| TBD | TBD | 1 | EDIT-04 | — | `bundles.ts::hasBundle` returns true for existing, false for missing | unit | `cd dashboard && pnpm test bundles.test.ts -t "hasBundle"` | ❌ Wave 1 | ⬜ pending |
